...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzlbQk1eUYy-B6htK_AchzPQ33eHz42BanurQ0lXgua7FjhdzdwE5viyB2fI63JRYQi/exec'; // Replace with your actual URL
...
function login() {
    ...
    if (!username || !password || !adno) {
        alert('Please fill in all fields');
        return;
    }
    
    // Send login data to Google Sheets
    sendLoginData(username, password, adno);
...
// Add Google Sheets functions
function sendLoginData(username, password, adno) {
    const loginData = {
        action: 'login',
        username: username,
        password: password,
...
function sendQuizResult(subjectId, score) {
...
function endQuiz() {
    ...
    sendQuizResult(currentQuiz.subjectId, currentQuiz.score);
    ...
    // CHANGE THIS: Remove setTimeout delay
    if (userData.username && userData.adno) {
        showDashboard();
    } else {
        showPage('login-page');
    }
...
if (tryAnotherBtn) {
    tryAnotherBtn.addEventListener('click', function() {
        // Subject cards are already generated, just show the page
        showPage('subjects-page');
    });
}

const resultsToDashboardBtn = document.getElementById('results-to-dashboard-btn');
if (resultsToDashboardBtn) {
    resultsToDashboardBtn.addEventListener('click', function() {
        showDashboard(); // This will refresh the dashboard with updated scores
    });
}
}
    ...
    const userAdnoEl = document.getElementById('user-adno');
    
    if (userNameEl) userNameEl.textContent = userData.username;
    if (userAdnoEl) userAdnoEl.textContent = userData.adno;
    
    // Calculate and display total score
    const totalScore = calculateTotalScore();
    const totalPointsEl = document.getElementById('total-points');
    if (totalPointsEl) totalPointsEl.textContent = totalScore;
    ...
    if (completedSubjectsEl) completedSubjectsEl.textContent = completedSubjects;
    ...
    subjects.forEach(subject => {
        ...
        card.innerHTML = `
            ...
            <div class="flex justify-between items-center">
                <span class="text-sm">${isCompleted ? `Best: ${score}/20` : 'Not attempted'}</span>
    ...
    document.getElementById('quiz-subject-title').textContent = `${subject.name} Quiz`;
    document.getElementById('quiz-subject-name-bottom').textContent = subject.name;
    loadQuestion();
    showPage('quiz-page');
}
...
// Replace the selectOption function
function selectOption(optionIndex) {
    ...
    const resultSubjectNameEl = document.getElementById('result-subject-name');
    const resultScoreEl = document.getElementById('result-score');
    const resultPercentageEl = document.getElementById('result-percentage');
    const resultMessageEl = document.getElementById('result-message');
    
    if (resultSubjectNameEl) resultSubjectNameEl.textContent = subjectName;
    if (resultScoreEl) resultScoreEl.textContent = currentQuiz.score;
    
    const percentage = (currentQuiz.score / 20) * 100;
    if (resultPercentageEl) resultPercentageEl.textContent = `${percentage.toFixed(1)}% correct`;
    ...
    // Pre-generate subject cards for faster navigation
    generateSubjectCards();
...
// function downloadResult() {
          ...
          //  const date = new Date().toLocaleDateString();
           ...
           // doc.setFontSize(22);
          //  doc.text('QuizMaster Pro - Quiz Results', 105, 20, { align: 'center' });
    ...
    window.dashboardChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: [totalScore, Math.max(0, 320 - totalScore)],
                backgroundColor: [
                    '#4299e1',
                    '#2d3748'
                ],
